...
**Write the sourced research to a new file:** `career/[year]/offseason/initial_roster_and_cap_sheet.md` — kept inside the career folder rather than `library/` so the owner doesn't have to search elsewhere for the team's own starting snapshot. Cite sources the same way the library files do.
