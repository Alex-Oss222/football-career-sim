...
6. Verify Offseason Status fields are present and parseable
7. Confirm active consequences are parseable
8. Check for missing required fields → prompt user to fill gaps
9. Load tape mode and position if Mode A
10. If Time Bubble Active = Yes, resume at saved Bubble Stage (do not advance calendar)
11. Resume from next turn
```
...
8. Update Offseason Status:
   - Set Club Decision to Promoted.
   - Set Transition Pending to Yes.
   - Set Promotion Readiness to N/A (reset for new role).
9. After the transition turn resolves, set Transition Pending to No.
```
...
- **Offseason Status is mandatory during offseason.** This section tracks the time bubble, evaluation results, and club decision. It prevents the sim from skipping or duplicating offseason steps on resume.
...
- **Era Ruleset is persistent.** Once set at simulation start, the era ruleset does not change unless the in-game calendar crosses a historical rule-change boundary. It is always readable in the Offseason Status section.

---

*State Block — Complete. Always active. Provides persistence layer for Player Module, Coaching Module, Offseason Evaluation & Job Cycle, and Interview Module.*