...
> See `eagles_wco_install_basics_1999.md` for Eagles-specific WCO learning sequence.
...
### What Stone Knows (January 1999)

Stone enters coaching knowing football from the **player's perspective** — specifically as a tight end in a Coryell-influenced vertical passing offense.

```
FROM PLAYING CAREER:
  - Route concepts from the TE/WR perspective (how to run them, not how to design them)
  - Blocking schemes he executed (not the full OL picture)
  - Defensive coverages he read as a player (Cover 1, 2, 3, 4 — recognition, not teaching)
...
```
  - How to break down film for someone else
  - How to design a practice plan
  - How to install a scheme
  - How plays are called, sequenced, or game-planned
  - The full picture of offensive line assignments
...
```
  ...
  Defensive Systems:                 6/10 — read them as a player, never coached them
...
```
  - Break down opponent film into tendency reports
  - Run the scout team to simulate upcoming opponents
  - Handle administrative tasks (printing playbooks, organizing film cuts, logistics)
  - Observe and learn how coaches teach, plan, and communicate
```

### What He's Building

```
  - Understanding of how a coaching staff operates day-to-day
  - How meetings are structured and run
  - How practice periods are organized and timed
  - How game plans are assembled from scouting reports
  - Relationships with the staff — earning trust through work, not reputation
```

### Film Perspective Shift (In Progress)

Stone must learn to watch film as a **coach**, not a player:
...
This shift is the single most important development task at Level 0. Everything else in coaching depends on it.

### System Exposure

Stone absorbs whatever system his first team runs. He has no preference yet because he has no coaching frame of reference. His playing background gives him a lens (Air Coryell concepts, vertical passing, TE versatility), but he hasn't learned to coach any system.
