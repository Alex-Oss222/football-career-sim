...
**Resolution-packet freeze, added 2026-09-18, at the user's explicit instruction after a review identified that "seeded" randomness with no rule for who picks the seed, or when, is not actually an anti-bias mechanism — a seed chosen after the resolving process has formed a preferred outcome is bias wearing a randomness costume.** Before any drive, turnover/penalty/injury check (§3.3), or other consequential draw is generated, the resolver must assemble the canonical resolution packet containing: an event ID, the world-state snapshot it was computed from, the normalized inputs (per §3.2 step 2's rule — concrete facts, not persuasive framing), the resulting modifiers, the outcome distribution those modifiers produce, and the engine-procedure version. The private Engine Ledger then commits the packet's canonical SHA-256 identity together with the event ID *before* any draw is generated. That cryptographic commitment is the closed packet record: the same event ID may replay only against the same digest, and an altered digest is refused. A correction must reconstruct the packet from the original pre-resolution state and procedure, reproduce the committed digest exactly, and only then rerun; if reconstruction does not match, the correction fails closed. The private career seed remains independently persisted and non-discretionary, and no result may be regenerated by changing either seed or packet identity after the outcome is known.
...
Extends §6.2's pattern to trades rather than restating it. A trade is not confined to the free-agency window — it can happen at the draft, during free agency, or around the real in-season trade deadline for the year in play (Document 2 §6) — so it is tracked in its own `career/<year>/trades/` folder, not nested under `offseason/` (see `career/README.md`).

1. **Coach's plan.** The coach states what he would move and what he is looking for, plus any hard limits, in one consolidated turn — recorded in `trade_targets.md` before any outcome is known, never rewritten to match the eventual result.
2. **Every other club's own trade activity resolves autonomously**, per Document 2 §12 — the coach learns about a trade between two non-protagonist clubs only through a plausible channel (Document 1 §7), the same as any other transaction he isn't party to.
3. **Inbound proposals are logged as they arrive, separately from the coach's own wish list.** A real offer from another club — whether it answers the coach's stated plan or comes in unprompted — is recorded, dated, in `trade_offers.md` while it's under discussion, never presented as a completed transaction until it actually closes.
4. **A trade involving the protagonist's team is a genuine negotiation, not a scripted outcome** — the other club's willingness, asking price, and counter-interest are real market pressure the coach can fail to meet, exactly as §6.2 already establishes for free agency.
5. **Outcome report and immediate accounting.** One consolidated result per plan or offer (completed at what terms; talks fell through and why; no interest found), moved from `trade_offers.md` into `trades.md` only once it actually closes, with the roster, depth chart, and cap/draft-capital accounting in `state/04_Roster_and_Staff_Register.md` and Document 4 updated the instant the trade closes, per §6.3 below — never on the next weekly cycle.
