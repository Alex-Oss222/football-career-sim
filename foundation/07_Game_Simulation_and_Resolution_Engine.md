...
**Last Document 7 content-changing update:** `2026-09-18 - substantive correction after a detailed user-supplied anti-favoritism review: §1's scope-discipline rule no longer runs two different outcome adjudicators (structured engine for the protagonist, free-form narrative judgment for background games); every game now samples from the same rating anchors/matchup-deltas/seeded draw, with only narration depth varying by stakes. §3.2 step 2 gained a normalization rule (a scheme-mismatch modifier is earned by a concrete checkable claim, never by persuasive phrasing). §3.4 gained a resolution-packet freeze (inputs/modifiers/distribution/seed committed and logged before the draw, never revised toward a more desirable result) and a non-discretionary seed-derivation rule. These mechanically enforce new Document 1 §9.1. Prior update, same day: §3.7 defers to the playbook's own embedded simulation-rules sections. Prior update, same day: Codex delegation note in §1 and §12 pointing at AGENTS.md. Prior update (2026-09-17): created and locked the same day; see version history in git for the full sequence. No section's core design intent changed, only its enforceability.`
